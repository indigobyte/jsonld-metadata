...
import org.schema.*
class MutableEndorseAction {
  private val map = HashMap<String,Any>()
  var endorsee: Any
    get() = map["endorsee"]!!
    set(value) { map["endorsee"] = value }
  fun endorseeOrganization(builder: MutableOrganization.() -> Unit) { map["endorsee"] = MutableOrganization().apply(builder).build() }
  fun endorseePerson(builder: MutablePerson.() -> Unit) { map["endorsee"] = MutablePerson().apply(builder).build() }
  var agent: Any
    get() = map["agent"]!!
  ...
  fun build() = org.schema.EndorseAction.Builder(map).build()
}
