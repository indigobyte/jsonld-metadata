...
import org.schema.*
class MutableSourceCodeReview {
  private val map = HashMap<String,Any>()
  var state: ReviewState
    get() = map["state"] as ReviewState
    set(value) { map["state"] = value }
  var reviewer: Person
    get() = map["reviewer"] as Person
    set(value) { map["reviewer"] = value }
  fun reviewer(builder: MutablePerson.() -> Unit) { map["reviewer"] = MutablePerson().apply(builder).build() }
  var watcher: Person
    get() = map["watcher"] as Person
    set(value) { map["watcher"] = value }
  fun watcher(builder: MutablePerson.() -> Unit) { map["watcher"] = MutablePerson().apply(builder).build() }
  ...
  fun build() = org.schema.SourceCodeReview.Builder(map).build()
}
