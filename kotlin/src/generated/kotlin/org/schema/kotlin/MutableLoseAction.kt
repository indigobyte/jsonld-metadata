...
import org.schema.*
class MutableLoseAction {
  private val map = HashMap<String,Any>()
  var winner: Person
    get() = map["winner"] as Person
    set(value) { map["winner"] = value }
  fun winner(builder: MutablePerson.() -> Unit) { map["winner"] = MutablePerson().apply(builder).build() }
  var agent: Any
    get() = map["agent"]!!
  ...
  fun build() = org.schema.LoseAction.Builder(map).build()
}
