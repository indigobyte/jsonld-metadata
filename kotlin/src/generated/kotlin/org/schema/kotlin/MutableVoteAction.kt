...
import org.schema.*
class MutableVoteAction {
  private val map = HashMap<String,Any>()
  var candidate: Person
    get() = map["candidate"] as Person
    set(value) { map["candidate"] = value }
  fun candidate(builder: MutablePerson.() -> Unit) { map["candidate"] = MutablePerson().apply(builder).build() }
  var actionOption: Any
    get() = map["actionOption"]!!
    set(value) { map["actionOption"] = value }
  fun actionOptionThing(builder: MutableThing.() -> Unit) { map["actionOption"] = MutableThing().apply(builder).build() }
  var agent: Any
    get() = map["agent"]!!
  ...
  fun build() = org.schema.VoteAction.Builder(map).build()
}
