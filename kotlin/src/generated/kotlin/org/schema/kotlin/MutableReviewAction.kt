...
import org.schema.*
class MutableReviewAction {
  private val map = HashMap<String,Any>()
  var resultReview: Review
    get() = map["resultReview"] as Review
    set(value) { map["resultReview"] = value }
  fun resultReview(builder: MutableReview.() -> Unit) { map["resultReview"] = MutableReview().apply(builder).build() }
  var agent: Any
    get() = map["agent"]!!
  ...
  fun build() = org.schema.ReviewAction.Builder(map).build()
}
