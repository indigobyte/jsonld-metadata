...
import org.schema.*
class MutableFastFoodRestaurant {
  ...
  var starRating: Rating
    get() = map["starRating"] as Rating
    set(value) { map["starRating"] = value }
  fun starRating(builder: MutableRating.() -> Unit) { map["starRating"] = MutableRating().apply(builder).build() }
  ...
  fun build() = org.schema.FastFoodRestaurant.Builder(map).build()
}
