...
import org.schema.*
class MutableDataCatalog {
  private val map = HashMap<String,Any>()
  var dataset: Dataset
    get() = map["dataset"] as Dataset
    set(value) { map["dataset"] = value }
  fun dataset(builder: MutableDataset.() -> Unit) { map["dataset"] = MutableDataset().apply(builder).build() }
  ...
  fun build() = org.schema.DataCatalog.Builder(map).build()
}
