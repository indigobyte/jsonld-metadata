...
class MutableBlog {
  ...
  var blogPost: BlogPosting
    get() = map["blogPost"] as BlogPosting
    set(value) { map["blogPost"] = value }
  fun blogPost(builder: MutableBlogPosting.() -> Unit) { map["blogPost"] = MutableBlogPosting().apply(builder).build() }
  var issn: Identifier
  ...
  fun build() = org.schema.Blog.Builder(map).build()
}
