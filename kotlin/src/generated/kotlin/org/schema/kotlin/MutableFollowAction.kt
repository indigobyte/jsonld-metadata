...
import org.schema.*
class MutableFollowAction {
  private val map = HashMap<String,Any>()
  var followee: Any
    get() = map["followee"]!!
    set(value) { map["followee"] = value }
  fun followeeOrganization(builder: MutableOrganization.() -> Unit) { map["followee"] = MutableOrganization().apply(builder).build() }
  fun followeePerson(builder: MutablePerson.() -> Unit) { map["followee"] = MutablePerson().apply(builder).build() }
  var agent: Any
    get() = map["agent"]!!
  ...
  fun build() = org.schema.FollowAction.Builder(map).build()
}
