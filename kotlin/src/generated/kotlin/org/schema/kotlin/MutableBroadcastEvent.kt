...
import org.schema.*
class MutableBroadcastEvent {
  private val map = HashMap<String,Any>()
  var videoFormat: String
    get() = map["videoFormat"] as String
    set(value) { map["videoFormat"] = value }
  var broadcastOfEvent: Event
    get() = map["broadcastOfEvent"] as Event
    set(value) { map["broadcastOfEvent"] = value }
  fun broadcastOfEvent(builder: MutableEvent.() -> Unit) { map["broadcastOfEvent"] = MutableEvent().apply(builder).build() }
  var isLiveBroadcast: Boolean
    get() = map["isLiveBroadcast"] as Boolean
    set(value) { map["isLiveBroadcast"] = value }
  var isAccessibleForFree: Boolean
    get() = map["isAccessibleForFree"] as Boolean
    set(value) { map["isAccessibleForFree"] = value }
  var publishedOn: BroadcastService
  ...
  fun build() = org.schema.BroadcastEvent.Builder(map).build()
}
